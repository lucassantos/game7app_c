...
var openFB = (function () {
    ...
    function login(callback, options) {
        ...
        loginWindow = window.top.open(loginURL + '?client_id=' + fbAppId + '&redirect_uri=' + redirectURL +
    ...
    function logout(callback) {
        ...
        if (token) {
            logoutWindow = window.top.open(logoutURL + '?access_token=' + token + '&next=' + logoutRedirectURL, '_blank', 'location=no,clearcache=yes');
            if (runningInCordova) {
                setTimeout(function() {
                    logoutWindow.close();
                }, 700);
